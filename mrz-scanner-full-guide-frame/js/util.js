...
export function extractDocumentFields(result) {
  ...
  const rawFieldWithStatus = (fieldName) => ({
    text: result.getFieldRawValue(fieldName),
    status: result.getFieldValidationStatus(fieldName),
  });
  ...
  return {
    ...
    Nationality: rawFieldWithStatus("nationality"),
    "Document Number": getDocumentNumber(documentType),
    "Issuing State": rawFieldWithStatus("issuingState"),
